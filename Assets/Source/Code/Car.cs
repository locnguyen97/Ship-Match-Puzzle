...
public class Car : MonoBehaviour
{
    ...
    void CheckAndMove()
    {
        ...
        {
            if (GameManager.Instance.GetCurLevel().gameObjectsSlot.Count > 0)
            {
                var target = GameManager.Instance.GetCurLevel().gameObjectsSlot[0];
                GameManager.Instance.GetCurLevel().gameObjectsSlot.RemoveAt(0);
                Move(target.transform,false);
            }
            else
            {
